...
test('Core V3 Storage Mechanics', async (t) => {
  ...
  // 1. Initial State
  t.equal(memfs.existsSync('/src/main.js'), false, 'File should not exist initially');

  // 2. Write File
  await memfs.writeFileSync('/src/main.js', testContent);
  t.equal(memfs.existsSync('/src/main.js'), true, 'File should exist after writing');

  // 3. Verify Content Retrieval via MemFS
  const readContent = await memfs.readFileSync('/src/main.js');
  ...
  t.equal(fileNode.blobId, blobInfo.blobId, 'FileNode blobId correctly maps to the dedicated BlobStore hash');
});
