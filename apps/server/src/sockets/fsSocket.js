...
export const setupFsSocket = (io, socket) => {
  ...
  socket.on('fs:set_workspace', async (payload, callback) => {
    try {
      const { path: newPath } = payload;
      if (!newPath) throw new Error('No path provided');

      // Pivot the backend workspace
      const verifiedPath = await changeWorkspace(newPath);
      console.log(`[FsSocket] Workspace restored from cache to: ${verifiedPath}`);
      ...
      console.error(`[FsSocket] fs:set_workspace failed:`, error.message);
      if (callback) callback({ success: false, error: error.message });
    }
  });
};
