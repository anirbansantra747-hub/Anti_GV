...
function getAvailableModels(poolNames, healthState, forceFallback = false) {
  return poolNames
    ...
    .filter((model) => {
       ...
       const isHealthy = isProviderAvailable(model.provider) || healthState?.[model.provider]?.circuitState === 'half_open';
...
export function selectRoute(taskType, overrides = {}) {
  ...
  const allPrimaryPressured = primaryPool.length > 0 && primaryPool.every(m => activeRateLimiter.isApproachingLimit(m.provider));
  
  let candidates = [];
  if (allPrimaryPressured) {
      console.warn(`[TaskRouter] Primary pool is rate-pressured. Shifting traffic to fallback pool for ${taskType}`);
